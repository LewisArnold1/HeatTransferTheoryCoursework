...
def format_number(value):
    """ Formats a number to 5 significant figures. """
    return f"{value:.5g}"

def run_sweep():
    """ Runs a sweep over different initial values of T_s and T_c and saves results to CSV. """
    
    # Define the range of initial guesses for T_s and T_c
    T_s_values = np.linspace(25, 40, 10)  # 10 values between 25°C and 40°C

    # Sweep through different initial values of T_s and T_c
    for T_s_initial in T_s_values:
        T_c_initial = (T_s_initial + T_a)/2
        print(f"Running simulation for T_s_initial = {T_s_initial:.2f}°C, T_c_initial = {T_c_initial:.2f}°C")

        # Solve for final T_s and T_c
        T_s_final, T_c_final = compute_Tc(T_s_initial, T_c_initial)

        # Compute final U_overall and heat transfer coefficients for reference
        h_c_sc, h_r_sc, h_c_ca, h_r_ca, U_overall = compute_U(T_s_final, T_c_final, T_a)
        T_out_final = outlet_temperature(U_overall, T_s_final)

        # Store results
        sweep_results.append([
                format_number(T_s_initial), format_number(T_c_initial),
                format_number(T_s_final), format_number(T_c_final), format_number(T_out_final),
                format_number(h_c_sc), format_number(h_r_sc), format_number(h_c_ca),
                format_number(h_r_ca), format_number(U_overall)
            ])

    # Convert results to DataFrame
    df_sweep = pd.DataFrame(sweep_results, columns=[
        "Initial T_s (°C)", "Initial T_c (°C)", "Final T_s (°C)", "Final T_c (°C)", "Final T_out (°C)",
        "h_c_sc", "h_r_sc", "h_c_ca", "h_r_ca", "U_overall"
    ])

    # Define file path for saving the results
    sweep_file_path = "/Users/lewisarnold1/Desktop/Documents/Uni/Year 5/ES4D9 - Heat Transfer Theory and Design/Assignment/Results/sweep.xlsx"

    # Save to CSV
    df_sweep.to_excel(sweep_file_path, index=False)

    print(f"\nSweep results saved to {sweep_file_path}")
